...
<body>
    ...
    <div class="flexcommon">
        ...
        <p>Bij elk punt waar jullie komen, kunnen jullie de locatie laten controleren en kleine vragen krijgen voor jullie volgende locatie.</p>
        ...
        <?php if ($user["can_start"]) : ?>
            <button class="check" onclick="check(<?= $user["checkpoint"] ?>, <?= $route ?>, <?= $user["id"] ?>);">Ga door</button>
        <?php else : ?>
            <p class="msgerror">Wacht even tot Sander je laat starten!</p>
        <?php endif ?>
        <div id="errors"></div>
    </div>
